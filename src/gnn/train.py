...
def _apply_norm(sample: GraphSample, mean: np.ndarray | None, std: np.ndarray | None) -> GraphSample:
    if mean is None or std is None:
        return sample
    x = (sample.x - torch.from_numpy(mean)) / torch.from_numpy(std)
    return GraphSample(sample.image_id, x.float(), sample.edge_index, sample.y, sample.supervision_mask, sample.eval_mask)
...
def _evaluate_split(model: torch.nn.Module, samples: list[GraphSample], device: torch.device) -> tuple[dict, dict[str, dict], np.ndarray, dict[str, list[float]]]:
    model.eval()
    cases = []
    num_total = 0
    num_supervised = 0
...
def run_training(splits: dict[str, list[GraphSample]], output_root: str | Path, experiment_name: str, cfg: TrainConfig, graphs_root: str | None = None) -> Path:
    ...
    seg_idx = (int(fmap["seg_probs_mean"][0]), int(fmap["seg_probs_mean"][0]) + 4)

    norm_mean, norm_std = (None, None)
    if cfg.normalize_features:
        norm_mean, norm_std = _fit_normalization(splits["train"])
        splits = {k: [_apply_norm(s, norm_mean, norm_std) for s in v] for k, v in splits.items()}
    ...
    for epoch in epoch_iter:
        ...
        eval_val, _, _, _ = _evaluate_split(model, splits["val"], device)
        cur_tuple = (
        ...
        with log_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(json_safe({"epoch": epoch, "train_loss": (loss_acc / max(steps, 1)), "val": eval_val})) + "\n")
    ...
    metrics_summary = {"split_metrics": {}, "best_epoch": best_epoch, "selection_metric": cfg.selection_metric}
    ...
    for split in ("train", "val", "test"):
        split_metrics, case_rows, cm, arrays = _evaluate_split(model, splits[split], device)
    ...
    run_config.update({"graph_root": graphs_root, "feature_dim": in_dim, "feature_index_map": fmap, "best_epoch": best_epoch, "class_weight_policy": "inverse_frequency" if cfg.use_class_weights else "none"})
