...
# Loss helpers are routed through src.trainers.loss_utils below.
...
def main() -> None:
    ...
    if cfg_model == "pspnet":
        ...
        if soft_loss_type != "ce":
            raise ValueError(
                f"soft_label_loss must be 'ce', got {soft_loss_type!r}"
            )
        loss_variant = str(cfg.get("loss_variant", "soft_dice")).strip().lower()
        if loss_variant != "soft_dice":
            raise ValueError(
                f"loss_variant must be 'soft_dice', got {loss_variant!r}"
            )
    ...
    if cfg_model == "pspnet":
        logger.info(
            ...
            str(cfg.get("pspnet_loss_mode", "gleason_ce_soft")).strip().lower(),
    ...
    if pspnet_loss_mode != "gleason_ce_soft":
        raise ValueError(
            f"pspnet_loss_mode must be 'gleason_ce_soft', got {pspnet_loss_mode!r}"
        )
    pspnet_soft_term = str(cfg.get("pspnet_soft_term", "ce")).strip().lower()
    if pspnet_soft_term != "ce":
        raise ValueError(f"pspnet_soft_term must be 'ce', got {pspnet_soft_term!r}")
